...
export default function GameTypes() {
  ...
  // access
  const isAddAccess = usePermission('Game', 'Game Types', 'Add')
  const isEditAccess = usePermission('Game', 'Game Types', 'Edit')
  const isDeleteAccess = usePermission('Game', 'Game Types', 'Delete')
  ...
  const getTypeList = async () => {
    try {
      if (token) {
        setDataLoading(true)
        const res = await DataServices.callGetAllGameType(token)
  ...
  const handleDelete = async id => {
    try {
      const res = await DataServices.callRemoveGameType(id, token)
      if (res?.data?.status === true) {
        getTypeList()
      } else {
        toast.error(res?.data?.message)
  ...
  const onSubmit = async data => {
    try {
      ...
      if (showAddEditForm.isEdit) {
        res = await DataServices.callEditGameType(id, data, token)
      } else {
        res = await DataServices.callAddGameType(data, token)
      }
      ...
      <div className='mt-3'>
        {showAddEditForm.isVisible ? (
          <AddEditForm
            ...
            loading={loading}
            title={showAddEditForm.isEdit ? 'Edit Game Type' : 'Add New Game Type'}
          />
        ) : (
          <Datagrid
            data={data}
